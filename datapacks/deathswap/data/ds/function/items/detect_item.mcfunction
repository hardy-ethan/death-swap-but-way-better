$$(a)light_blue$(b)"Give a player speed 1 billion: 40 secs"$(c)1a
$$(a)gray$(b)"Give yourself materials to build a wither"$(c)2a
$$(a)orange$(b)"Shield yourself from negative items: 2 mins"$(c)3a
$$(a)purple$(b)"Teleport someone to the End"$(c)4a
$$(a)yellow$(b)"Swap all players right NOW!"$(c)5a
$$(a)pink$(b)"Teleport someone really far away"$(c)6a
$$(a)light_blue$(b)"Give yourself a super fast shovel"$(c)7a
$$(a)brown$(b)"Spawn 100 villagers on someone"$(c)8a
$$(a)light_gray$(b)"Summon an 300 block-tall gravel tower"$(c)9a
$$(a)blue$(b)"Teleport someone to the middle of the ocean"$(c)10a
$$(a)lime$(b)"Disable a player's ability to jump: 60 secs"$(c)11a
$$(a)red$(b)"Place a Nether Portal next to you"$(c)12a
$$(a)brown$(b)"Give yourself 32 steak"$(c)13a
$$(a)red$(b)"Spawn TNT on someone"$(c)14a
$$(a)white$(b)"Place a 7x7x7 cube of air on someone"$(c)15a
$$(a)orange$(b)"Teleport someone to y= -60"$(c)16a
$$(a)gray$(b)"Spawn a hole that goes to the void"$(c)17a
$$(a)magenta$(b)"Enter creative mode for 10 seconds"$(c)18a
$$(a)red$(b)"Give yourself 4 pieces of TNT"$(c)19a
$$(a)white$(b)"Trap a player in a barrier block cage"$(c)20a
$$(a)green$(b)"Give a player motion sickness: 30 seconds"$(c)21a
$$(a)light_gray$(b)"Turn all nearby blocks of a player to stone"$(c)22a
$$(a)brown$(b)"Put a curse of binding leather chestplate on someone"$(c)23a
$$(a)orange$(b)"/clear a RANDOM person's inventory (could be you!!)"$(c)24a
$$(a)gray$(b)"Make someone leave a bedrock trail: 40 secs"$(c)25a
$$(a)gray$(b)"Give someone blindness & darkness: 40 secs"$(c)26a
$$(a)light_gray$(b)"Give yourself one ravager spawn egg"$(c)27a
$$(a)red$(b)"Give yourself an elytra & fireworks"$(c)28a
$$(a)white$(b)"Give yourself a milk bucket to clear effects"$(c)29a
$$(a)orange$(b)"Give yourself a fire resistance potion"$(c)30a
$$(a)orange$(b)"Shield yourself from negative items: 3 mins"$(c)31a
$$(a)light_gray$(b)"Spawn falling anvils above someone"$(c)32a
$$(a)cyan$(b)"Put someone in adventure mode: 40 secs"$(c)33a
$$(a)yellow$(b)"Place bells all around someone"$(c)34a
$$(a)orange$(b)"Place a layer of lava above someone"$(c)35a
$$(a)yellow$(b)"Spawn a horde of bees around someone"$(c)36a
$$(a)light_blue$(b)"Give yourself full diamond armor & tools"$(c)37a
$$(a)purple$(b)"Teleport to someone (& surprise attack them?)"$(c)38a
$$(a)cyan$(b)"Invert someone's movement keys: 45 secs"$(c)39a
$$(a)orange$(b)"Strike someone with lightning"$(c)40a
$$(a)orange$(b)"Put a pumpkin head on someone: 1 min"$(c)41a
$$(a)cyan$(b)"Drown someone with a flood of water"$(c)42a
$$(a)brown$(b)"Make someone extremely tiny"$(c)43a
$$(a)lime$(b)"Make someone extremely huge"$(c)44a
$$(a)yellow$(b)"Give yourself a totem of undying"$(c)45a
$$(a)light_gray$(b)"Place a layer of gravel above EVERYONE except you"$(c)46a
$$(a)purple$(b)"Make the swap cycle length 30 seconds shorter"$(c)47a
$$(a)gray$(b)"Block out the sun for someone"$(c)48a
$$(a)lime$(b)"Fill up someone's inventory with junk"$(c)49a
$$(a)red$(b)"Transform the world around someone into the Nether"$(c)50a
$$(a)white$(b)"Spawn a bunch of cobwebs on someone"$(c)51a
$$(a)purple$(b)"Give yourself 3 ender pearls"$(c)52a
$$(a)brown$(b)"Spawn a village right where you are"$(c)53a
$$(a)orange$(b)"Spawn a desert temple right where you are"$(c)54a
$$(a)gray$(b)"Summon a California earthquake on EVERYONE except you"$(c)55a
$$(a)brown$(b)"Block someone from using any items: 3 mins"$(c)56a
$$(a)yellow$(b)"Give yourself an enchanted golden apple"$(c)57a
$$(a)green$(b)"Force a player to look down for 45 secs"$(c)58a
$$(a)white$(b)"Bombard someone with a ton of ghasts"$(c)59a
$$(a)lime$(b)"Jumpscare someone"$(c)60a
$$(a)red$(b)"Lock someone into a prison"$(c)61a
$$(a)cyan$(b)"Give yourself a water bucket"$(c)62a
$$(a)orange$(b)"Give yourself a lava bucket"$(c)63a
$$(a)orange$(b)"Give yourself a long fire resistance potion"$(c)64a
$$(a)cyan$(b)"Liquidate all blocks surrounding a player"$(c)65a
$$(a)white$(b)"Set time to midnight (or mid-day if it's night)"$(c)66a
$$(a)lime$(b)"Spawn a giant slime on someone"$(c)67a
$$(a)red$(b)"Give yourself 4 extra hearts for health"$(c)68a
$$(a)red$(b)"Take away 2 hearts from someone (leave them w/8)"$(c)69a
$$(a)green$(b)"Launch a Viet Cong ambush on someone"$(c)70a
$$(a)light_blue$(b)"Give yourself a one-hit-kill sword"$(c)71a
$$(a)lime$(b)"Switch game's language to Chinese 中文"$(c)72a
$$(a)gray$(b)"Set /difficulty to hard"$(c)73a
$$(a)orange$(b)"Set /difficulty to normal"$(c)74a
$$(a)lime$(b)"Set /difficulty to easy"$(c)75a
$$(a)green$(b)"Teleport someone to a superflat world"$(c)76a
$$(a)brown$(b)"Give yourself a crafting table & furnace"$(c)77a
$$(a)brown$(b)"Crash somebody's Minecraft game"$(c)78a
$$(a)brown$(b)"Turn OFF Keep Inventory (or ON if its off)"$(c)79a
$$(a)white$(b)"Put someone into spectator mode: 20 secs"$(c)80a
$$(a)pink$(b)"Re-shuffle everyone around the world EXCEPT YOU"$(c)81a
$$(a)magenta$(b)"Place an Amethyst Geode where you are"$(c)82a
$$(a)red$(b)"Turn OFF /gamerule natural regeneration (or ON if its off)"$(c)83a
$$(a)white$(b)"Build a Quartz maze around someone"$(c)84a
$$(a)gray$(b)"Turn all blocks near someone to obsidian"$(c)85a
$$(a)light_gray$(b)"Build a Stalagmite trap next to you"$(c)86a
$$(a)red$(b)"Delete the Chunk someone's standing in"$(c)87a
$$(a)yellow$(b)"Switch places with someone"$(c)88a
$$(a)light_blue$(b)"Mine blocks faster for 90 seconds"$(c)89a
$$(a)purple$(b)"Make the game low-gravity: 3 minutes"$(c)90a
$$(a)lime$(b)"Take NO fall damage for the next 5 minutes"$(c)91a
$$(a)purple$(b)"Make someone's ears bleed (HEADPHONE WARNING!)"$(c)92a
$$(a)magenta$(b)"Summon a Stronghold below you"$(c)93a
$$(a)orange$(b)"Give yourself a flint & steel"$(c)94a
$$(a)green$(b)"Put someone into Parkour Civilization (from Youtube)"$(c)95a
$$(a)cyan$(b)"Secretly spy on what someone's currently doing"$(c)96a
$$(a)orange$(b)"Shield yourself from negative items: 2.5 mins"$(c)97a
$$(a)brown$(b)"Block someone from using crafting tables: 90 sec"$(c)98a
$$(a)magenta$(b)"Gain a monster-proof forcefield: 3 mins"$(c)99a
$$(a)green$(b)"Spawn a Giant zombie on someone"$(c)100a
$$(a)lime$(b)"Place a Trial Chamber below you"$(c)101a
$$(a)brown$(b)"Build a Woodland Mansion where you are"$(c)102a
$$(a)yellow$(b)"Make someone continuously pee: 1 min"$(c)103a
$$(a)magenta$(b)"Get an enchanting table, lapis, & XP"$(c)104a
$$(a)cyan$(b)"Summon the Warden on someone"$(c)105a
$$(a)red$(b)"Teleport someone directly to the Nether"$(c)106a
$$(a)red$(b)"Summon Oppenheimer's NUCLEAR bomb on someone"$(c)107a
$$(a)cyan$(b)"Summon an Ancient City below you"$(c)108a
$$(a)orange$(b)"Set everything around someone on fire"$(c)109a
$$(a)purple$(b)"Spawn 100 Endermen on someone"$(c)110a

# The text after $(c) shows the function in "ds:items/use/" that the dropped item with the name after $(b) and the dye color after $(a) is triggering