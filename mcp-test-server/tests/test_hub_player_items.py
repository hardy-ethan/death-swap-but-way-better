...
Changes tested:
  ...
  - tickHub() restores the mace if it is lost
  - Hub items (mace + wind charges) cannot be Q-dropped
  - Dying and respawning in hub restores the full item set

Test flow:
  1. Provision + start the server
  2. Spawn Alice (joining a HUB-phase server triggers onPlayerJoin -> sendToHub)
  3. Assert: Alice has a mace with the unbreakable component, exactly 16 wind
     charges, and no regeneration effect
  4. Clear Alice's wind charges; wait 4 s; assert tickHub topped them back to 16
  5. Clear Alice's mace; wait 2 s; assert tickHub restored it
  6. Attempt to Q-drop the mace and wind charges; assert both stay in inventory
  7. Kill Alice (/kill); wait for respawn; assert she has the full hub kit again
...
def main():
    ...
    # ---- Test 5: tickHub replenishes wind charges within 4 seconds --------
    ...
    ok = t1 and t2 and t3 and t4 and t5 and t6 and t7 and t8 and t9
    ...
    if not ok:
        ...
        if not t9:
            log(f"  FAIL: After respawn: mace={mace_after_respawn}, wc={wc_after_respawn}, regen={regen_after_respawn}")
